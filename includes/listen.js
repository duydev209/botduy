module.exports = function({ api, models }) {
 ...
 return async (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      //check body
      if (event.body == `QDuyy` || event.body == `qduyy` || event.body == `duyebox` || event.body == `${prefix}QDuyy`){
		  adminBot.forEach(e => {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Nhóm của bạn chưa được duyệt, hãy chat QDuyy để gửi yêu cầu duyệt đến admin`, event.threadID);
    }
    };
    switch (event.type) {
      //<--Nhận, xử lí dữ liệu-->//
