...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["kêu cac gi thế", "gii", "=))", "khà khà khà", "cuts", "uhh", "cơm nước gì chưa", "đây là đâu", "..", "them bu lul", "yamatee", "chit k", "hahha", "???", "t đóii", "cưới t đii", "cho hun cái=))", "ìn cha nạ", "hehee", "mầy là aii", ":((", "cọc còn gặp m nữa", "chụtt chụtt", "t dp kh", "yêu m=))", "bắn phi phai k?", "biết múa flo kh the", "éc écc", "huhhh", "buonn quá tr hiuhiu", "dạa", "si bi di di dom dom yet yet", "quãi chưởng thiệt ớ chớ", "đang ỉa đừng làm phiền", "cn cac", "t dp nhất box", "im coii", "i love you chu cà moa", "gì dạ 🙄", ":))", "m bị đin àa", "hí hí", "@@", "hảa", "làm gì vậyy", "ừa", "cac ba may", "đang buồn ỉa", "t die rồi nhé", "aaa", "tới luôn bác tài ơii", "jz eiu", "mô phật", "t là ai?", "bựa", "cười die", "._.", "met lòi loz", "hmm", "làm ni tui nhée", "đang phee", "ct nhaa ..", "me may bel" ,"suyy vai lul", "em còn trinhh..", "má m", "tuyển ny số lượng lớn", "tuyển ghệ đít bự"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("ngu mẹ m", threadID);
  };

  if ((event.body.toLowerCase() == "bot out")) {
    return api.sendMessage("out cl", threadID);
  };

   if ((event.body.toLowerCase() == "bot cút")) {
    return api.sendMessage("xàm", threadID);
  };

  if ((event.body.toLowerCase() == "bot lon")) {
    return api.sendMessage("lon me m", threadID);
  };

  if ((event.body.toLowerCase() == "bot óc chó")) {
    return api.sendMessage("cac", threadID);
  };

  if ((event.body.toLowerCase() == "bot chó") || (event.body.toLowerCase() == "bot chó")) {
    return api.sendMessage("á àa", threadID);
  };

  if ((event.body.toLowerCase() == "ôi") || (event.body.toLowerCase() == "ôi")) {
    return api.sendMessage("ôi loz", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ỏ") || (event.body.toLowerCase() == "oi")) {
    return api.sendMessage("hahaa", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "vip") || (event.body.toLowerCase() == "Vip")) {
    return api.sendMessage("vip con cac", threadID);
  };
