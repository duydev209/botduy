...
module.exports.onLoad = async () => {
    const dir = __dirname + `/cache/banchim/datauser/`;
    const _dir = __dirname + `/cache/banchim/datauser/`;
    const __dir = __dirname + `/cache/banchim/cache/`;
...
module.exports.checkPath = function (type, senderID) {
    ...
    const pathGame_1 = require("./cache/banchim/datauser/" + senderID + '.json');
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        writeFileSync( __dirname + `/cache/banchim/cache/banchim.png`, Buffer.from(download, "utf-8"));
        images.push(createReadStream(__dirname + `/cache/banchim/cache/banchim.png`));
    return images
}
    ...
    switch (args[0]) {
        ...
        case 'spin': {
            ...
            if(i == 3) {
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        dem.push(require(`./cache/banchim/datauser/${i}`));
                    }
                }
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users, getText}) => {
  ...
  const fs = require("fs");
    switch (handleReply.type) {
        case 'build': {
            var a = require(`./cache/banchim/datauser/${senderID}.json`);
