const {get:p}=require('axios'),i=url=>p(url,{responseType:'stream'}).then(r=>r.data),
      ...
      a=['https://i.imgur.com/4Hfduoe.png','https://i.imgur.com/EHsr9RL.png','https://i.imgur.com/Xuw6yG8.png'],b=['https://i.imgur.com/YPhfjfU.png','https://i.imgur.com/mahn5lm.png','https://i.imgur.com/cEivriJ.png'] 
module.exports.config={
    name:"kbb",version:"1.0.0",hasPermssion:0,
...
const {threadID:$,senderID:sd}=e,{sendMessage:s}=ap,{increaseMoney:$$,getData:g}=C,{getNameUser:nm}=U
  var kbb=['kéo','búa','bao'],rd=kbb[Math.floor(Math.random()*kbb.length)]
  ...
  switch(ar[0]){
  case'kéo':{var _=rd=='bao'?'thắng':rd=='búa'?'thua':'hòa',dn=_=='thắng'?`nhận : ${m}$\n> Hiện bạn còn : ${wn}$`:_=='thua'?`mất : ${m}$\n> Hiện bạn còn : ${ls}$`:`giữ lại : ${m}$\n> Hiện bạn còn : ${dr}$`,
  ...
  case'búa':{var _=rd=='kéo'?'thắng':rd=='bao'?'thua':'hòa',dn=_=='thắng'?`nhận : ${m}$\n> Hiện bạn còn : ${wn}$`:_=='thua'?`mất : ${m}$\n> Hiện bạn còn : ${ls}$`:`giữ lại : ${m}$\n> Hiện bạn còn : ${dr}$`,
            at=[await i(a[ra]),await i(_=='thắng'?w:_=='thua'?l:d),await i(b[rb])]
             await $$(sd,parseFloat(_=='thắng'?m:_=='thua'?-m:0));
             return s({body:`> Người chơi : ${n}\n> Lúc : ${tm}\n> Kết quả : ${_}\n> Bạn đưa ra : ${ar[0]}\n> Bot đưa ra : ${rd}\n> Bạn ${dn}`,attachment:at},$)}
  case'bao':{var _=rd=='búa'?'thắng':rd=='kéo'?'thua':'hòa',dn=_=='thắng'?`nhận : ${m}$\n> Hiện bạn còn : ${wn}$`:_=='thua'?`mất : ${m}$\n> Hiện bạn còn : ${ls}$`:`giữ lại : ${m}$\n> Hiện bạn còn : ${dr}$`,
            at=[await i(a[ra]),await i(_=='thắng'?w:_=='thua'?l:d),await i(b[rb])]
             await $$(sd,parseFloat(_=='thắng'?m:_=='thua'?-m:0));
             return s({body:`> Người chơi : ${n}\n> Lúc : ${tm}\n> Kết quả : ${_}\n> Bạn đưa ra : ${ar[0]}\n> Bot đưa ra : ${rd}\n> Bạn ${dn}`,attachment:at},$)}
  }
